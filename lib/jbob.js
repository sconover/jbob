Jbob = function(providedArgs) {
  var defaults = {tags:[], nonSelfClosingTags:[], attributeMappings:{}};
  var args = {};
  for (key in defaults) {
    if (providedArgs[key]) {
  ...
  function makeAttributesStr(attributes) {
    ...
    for (var key in attributes) {
      var attrName = args.attributeMappings[key] || key
      attrs.push(attrName + "=\"" + valueFor(attributes[key]) + "\"");
    }    
    attrs.sort();
  ...
  this.Root = function() {
    return flattenArray(argumentsAsArray(arguments)).join("")
  }
